var mineArray,  //地雷数组
	...
	startTime;  //开始时间
	//以下操作1表示揭开一个方块，操作2表示标记一个小旗，操作3表示标记一个问号
	//操作4表示若某个方块周围的地雷全都标记完，则将其周围剩下的方块挖开
$(function(){
		...
		if(inGame == 1) {
			if(e.which == 1) {
				...
				if(clicked.hasClass('flag')) {
					...
					if($('#gamecheck').attr('checked')) clicked.addClass('check');
					lastNum ++;
					countNum ++;
			...
			if(lastNum == countNum) endGame(1);  //因为最后剩下的方块均为雷时应直接结束游戏
			//因此设置为剩余雷数和未被揭开的方块数相等的时候结束游戏
		} else if(inGame == 2) {
    ...
    $('#gamemain').bind('contextmenu', function(){ return false; });  //阻止默认右击事件
...
function timer(){
	if(inGame == 1) {  //只在游戏进行中计时
		var now = new Date(),
			ms = now.getTime();
		$('#gametime').text(Math.ceil((ms - startTime) / 1000));
...
function endGame(isWin) {
	...
	$('#gamewarning').text(isWin ? 'You Win!' : 'You Lose!');
}